...
draw_matrix() {
    ...
    for ((i=1; i<=width; i++)); do
        ...
        c=${characters:RANDOM%${#characters}:1}
        
        # Afficher un caractère aléatoire en couleur
        echo -en "$color$c"
    done
}
...
while true; do
    # Appeler draw_matrix et afficher chaque ligne sans nouvelle ligne
    echo -ne "$(draw_matrix)\r"
done
